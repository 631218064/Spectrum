...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!stateId || Array.isArray(stateId)) {
    return res.status(400).json({ error: 'Invalid state ID', requestId });
  }
  ...
  try {
    ...
    logApiError(req, requestId, error, { stateId });
    res.status(500).json({ error: 'Failed to fetch cities', requestId });
  }
}
